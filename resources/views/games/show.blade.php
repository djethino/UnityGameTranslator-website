    ...
    <div class="flex items-center gap-4 sm:gap-6">
        ...
        <div class="min-w-0">
            ...
            {{-- What is known about the game, under the title because that is where somebody looks
                 ...
                 ⚠ No way to SAY it here: the first publisher says it on their publish screen, when
                 their upload creates the game (Game::declareAdultBy). What this page keeps is that
                 one person's way to take it back. --}}
            @if($game->adult)
                <div class="mt-2 flex flex-wrap items-center gap-2">
                    <p class="inline-flex items-center gap-1.5 px-2 py-0.5 rounded bg-gray-700/70 text-xs text-gray-300">
                    ...
                    @if($mayWithdrawAdult)
                        <form method="POST" action="{{ route('games.adult.withdraw', $game) }}">
                            @csrf
                            @method('DELETE')
                            <button type="submit"
                                class="px-2 py-0.5 text-xs rounded bg-gray-700 hover:bg-gray-600 text-gray-200 transition">
                                {{ __('games.adult.withdraw') }}
                            </button>
                        </form>
                    @endif
                </div>
