...
/**
 * Whether a game would accept a translation somebody typed here, with the result in front of
 * them: the frozen sequences verbatim, then the tokens as the same multiset — nothing missing,
 ...
 *
 * @returns {{accepted: boolean, errors: string[]}} the lines are the corpus's, verbatim.
 */
export function acceptsEdit(source, edited) {
    ...
    for (const sequence of frozenSequences(source)) {
        if (occurrences(edited, sequence) < occurrences(source, sequence)) {
            errors.push(`the exact sequence "${sequence}" is missing or altered`);
        }
    }
    ...
    for (const [token, expected] of Object.entries(inSource)) {
        ...
        if (found !== expected) errors.push(`token ${token} appears ${found} time(s) instead of ${expected}`);
    }

    for (const token of invented(source, edited)) errors.push(`token ${token} does not exist in the source`);
