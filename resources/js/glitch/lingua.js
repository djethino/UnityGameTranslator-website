...
/**
 * 🔴 Which vintage of the banks this page was rendered against, and why one word of it matters.
 *
 * Index N is the same line in every language — for ONE key list. Add a key to `en.json` and every
 * line after it shifts by one. The banks were cached for a day under a URL with no version in it, so
 * a visitor could hold Hindi from two days ago beside French from this morning, and the effect read
 * one at the other's index: "Rapide" is line 775 today, and on 31 August line 775 was the username
 * prompt. What appeared on the button was a real Hindi sentence, correctly fetched, and about
 * something else entirely.
 *
 * Two locks, because one of them is only a convention:
 *   the version goes in the query string, so a different key list is a different URL and the browser
 *     cannot serve one for the other;
 *   the response repeats the version it was built from, and a bank that disagrees is dropped — which
 *     holds even if something between here and the server ignores the query.
 */
const vintage = document.body.dataset.langBank || '';

async function load(locale) {
    if (banks.has(locale)) return banks.get(locale);
    try {
        const res = await fetch(`/lang-bank/${locale}.json?v=${encodeURIComponent(vintage)}`,
            { headers: { Accept: 'application/json' } });
        if (!res.ok) return null;
        const data = await res.json();
        // ⚠ Refused rather than adapted. There is no way to read a bank of another vintage safely,
        // and the wrong answer here is not a broken page — it is a plausible one, which is worse.
        if (vintage && data.v !== vintage) return null;
